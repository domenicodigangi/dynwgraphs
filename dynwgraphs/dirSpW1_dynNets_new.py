...
class dirSpW1_sequence_ss(dirSpW1_funs):
    ...
    def __init__(self, Y_T, X_T=None, phi_tv=True, ovflw_lm=True, distr='gamma',  dim_dist_par_un = 1, dist_par_tv=False, dim_beta = 1, beta_tv=[False], data_name="", 
    ...
    opt_options_ss_seq = {"opt_n" :"ADAM", "min_opt_iter" :200, "max_opt_iter" :5000, "lr" :0.01}):
        ...
        if X_T is None:
            ...
            if any(self.beta_tv):
                self.beta_T = nn.ParameterList([nn.Parameter(torch.zeros(dim_beta, self.n_reg)) for t in range(self.T)])
            else:
...
class dirSpW1_dynNet_SD(dirSpW1_sequence_ss):
    """
        Version With Score Driven parameters
    """

    def __init__(self, Y_T, X_T=None, phi_tv=True, ovflw_lm=True, distr='gamma',  dim_dist_par_un = 1, dist_par_tv=False, dim_beta = 1, beta_tv=[False], rescale_SD=True, init_sd_type = "unc_mean", data_name="", 
            opt_options_ss_sd = {"opt_n" :"ADAM", "min_opt_iter" :200, "max_opt_iter" :5000, "lr" :0.01}):

        super(dirSpW1_dynNet_SD, self).__init__( ovflw_lm, Y_T = Y_T, X_T=X_T, phi_tv=phi_tv, ovflw_lm=ovflw_lm, distr=distr,  dim_dist_par_un=dim_dist_par_un, dist_par_tv=dist_par_tv, dim_beta=dim_beta, beta_tv=beta_tv, data_name=data_name)
        
        self.opt_options_ss_sd = opt_options_ss_sd
    ...
    def update_dynw_par(self, t):
        ...
        if self.phi_tv:
            ...
            phi_t = self.identify(phi_t)

            phi_i_t, phi_o_t = splitVec(phi_t)
        
            s_i, s_o = splitVec(score_dict["phi"])  
            w_i, w_o = splitVec(self.sd_un_beta["w"])  
            B_i, B_o = splitVec(self.sd_un_beta["B"])  
            A_i, A_o = splitVec(self.sd_un_beta["A"])  
    ...
    def estimate_sd(self, tb_log_flag=True, par_init="fast_mle"):
        ...
        if self.phi_tv:
            par_to_opt.append(self.sd_un_phi)
        else:
            par_to_opt.append(self.phi_T[0])

        if self.dist_par_tv:
            par_to_opt.append(self.sd_un_dist_par_un)
        else:
            par_to_opt.append(self.dist_par_un_T[0])

        if self.beta_tv:
            par_to_opt.append(self.sd_un_beta)
        else:
            par_to_opt.append(self.beta_T[0])

        
        run_name = f"{self.data_name}_ScoreDriven"

        hparams_dict = {"phi_tv" :self.phi_tv, "dist_par_tv" :self.dist_par_tv, "beta_tv" :self.beta_tv, "par_init" :par_init, "init_sd_type" :self.init_sd_type}
